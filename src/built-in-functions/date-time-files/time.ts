  ...
  if (args.length === 1) {
    ...
    if (arg !== null && arg !== undefined) {
      if (typeof arg === 'string') {
        ...
        if (arg instanceof Date) {//@ts-ignore TODO: see .toISOString will probably be part of browser, but might cause issues. See to fix
    ...
    if (dtd) {
      try {
        ...
        if(err instanceof Error) throw new Error(`${err.message} - the fourth argument in "time" in-built function is expected to be of type "days and time duration"`);
      }
    }
    if (!t.isValid()) {
