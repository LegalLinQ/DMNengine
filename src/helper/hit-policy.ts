...
//import * as _ from "lodash"; //_.isEqual _.sortBy
import L_isEqual from "lodash/isEqual";
import L_sortBy from "lodash/sortBy";
...
const prepareOutputOrder = (output, priorityList) => {
  const arr = output.map((rule) => {
    const obj = {};//@ts-ignore
    obj.rule = rule;//@ts-ignore
    obj.priority = priorityList[rule];
    return obj;
  });
  const sortedPriorityList = L_sortBy(arr, ['priority']);
  const outputList = sortedPriorityList.map(ruleObj => ruleObj.rule);
  return outputList;
};
//commit: https://github.com/EdgeVerve/feel/commit/dec3d66f616b6fff5686e2511ccf97e04a0d9205
...
export { hitPolicyPass, getOrderedOutput };
